...
parameter_dir=sys.argv[1] #test dataset path
...
def Multi_prob(data,phi): #log p(f|phi^f)
    #print(data)
    phi +=1e-300
    #phi_log=np.log(phi)
    phi_log=phi
...
while per <= max_per:
    ...
    result_env=np.genfromtxt(parameter_dir+per_dir+"/Parameter.txt",delimiter=": ",dtype='S')#read parameter txt of test dataset
    train_dir=result_env[8][1]
    train_env=np.genfromtxt(train_dir+"/Environment_parameter.txt",delimiter=" ",dtype='S')#read environmet(room) parameter 
    ...
    test_num=0
    test_file=train_dir+"/test_num.txt" #txt written test data number
    test_data_num=test_data_read(test_file,test_num) #read test data number
    Out_put_dir=parameter_dir+per_dir+"/place_sampling/"
    ...
    pose=np.array(position_data_read_pass(train_dir,data_num,test_data_num))
    
    cost_sum=0
    p_num=0
    for p in test_data_num:
        if 45<=p and p<60 : #input range of new name index
            ...
            #print(name_vector)
            name_vector=np.array([0,0,0,0,0,0,0,0,1]) #name vector of 「田口」
            ...
            for r in xrange(len(pi[0])):
                for c in xrange(len(G)):
                    ...
                    #prob_rt[r] += prob
                    prob = Multi_prob(name_vector,phi_n[c])*G[c]*pi[c][r]
                    prob_rt[r] += prob
                #print(r,prob_rt[r])
